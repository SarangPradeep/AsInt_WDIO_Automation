...
describe('017-8_Assess Probability of Failure and determine final risk using EDD 1B', () => {

    it('should navigate to asset strategy development list view', async () => {
        await ASDListView.navigateToASDListView();
    });

    it('should be able to create asset strategy development for single equipment for EDD_XOM template', async () => {
        await ASDListView.createASDForEDDs_XOM();
    });

    it('should be able to capture asset strategy development details', async () => {
        await utils.captureHeaderDetails();
    });

    it('should be able to capture information from general section', async () => {
        await ASDDetailView.captureGeneralSelection(false);  
        //true if we want to select all general selection data, 
        // false if we want to select specific data
    });

    it('should select the required general selection data', async () => {
        await ASDDetailView.selectGeneralSelectionData
        ("Equipment Data","External AR/T","EDD 1B Ext Atmos Corr","CAP","Process Data");
    });
